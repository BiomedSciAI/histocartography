...
def predict_for_image(patch_info_coordinates,
                      image=None,
                      model_json=None,
                      model_weights=None,
                      visualize=1):
    ...
    y_pred_prob = model.predict_generator(
        datagen.flow(all_patches, batch_size=batch_size), steps=n_steps)
    ...
    if (visualize == 1):
        if not (image is None):
            ...
            downsample_factor = 4
            downsample_image = cv2.resize(
                image, (int(image.shape[1] / downsample_factor),
                        int(image.shape[0] / downsample_factor)))
            log.info('image shape: {}'.format(image.shape))
            log.info('downsample_image shape: {}'.format(
                downsample_image.shape))
            patch_info_coordinates = np.array(patch_info_coordinates)

            patch_info_coordinates_visualize = (
                np.array(patch_info_coordinates[:, 1:]) /
                downsample_factor).astype(int)
            pixel_values = np.array(y_pred_prob[:, 1] * 255).astype(
                int)  # probability of class 1, here ROI
            image_visualize = np.full(
                (int(image.shape[0] / downsample_factor),
                 int(image.shape[1] / downsample_factor)), 0, np.uint8)
            ...
            image_visualize_brg = cv2.cvtColor(image_visualize,
                                               cv2.COLOR_GRAY2RGB)
            overlap_heatmap = cv2.addWeighted(downsample_image, 0.4,
                                              image_visualize_brg, 0.6, 0)
