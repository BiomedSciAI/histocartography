...
class Patch_Evaluation:
    def __init__(self, config):
        ...
        # define model
        self.vae = dense_vae(embedding_dim=config.embedding_dim,
        ...
        # define optimizer
        self.optimizer = optim.Adam(self.vae.parameters(), lr=self.learning_rate, weight_decay=0.0001)
    ...
    def loss_function(self, x, x_reconstructed, mu, logvar):
        ...
        reconstuction_loss = torch.nn.BCELoss(reduction='sum')(x_reconstructed, x)/ x.shape[0]
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for (patch, mask, _) in tqdm(self.train_loader, unit='batch'):
                ...
                # mask input patches
                patch_masked = torch.mul(patch, mask) if self.is_mask else patch

                # flush gradients and run the model forward
                self.optimizer.zero_grad()

                x_reconstructed, mu, logvar = model(patch_masked)
                recon_loss, kl_loss, loss = self.loss_function(x=patch_masked, x_reconstructed=x_reconstructed, mu=mu, logvar=logvar)
            ...
            log_train_recon_loss = np.append(log_train_recon_loss, train_recon_loss)
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(model, self.model_save_path + 'vae_model_best_loss.pt')
            ...
            print(' - ' + str(int(time.time() - start_time)) + 's '
                  '- loss:', round(train_loss, 4), '- rl:', round(train_recon_loss, 4), '- kl:', round(train_kl_loss, 4),
                  '- val_loss:', round(val_loss, 4), '- val_rl:', round(val_recon_loss, 4), '- val_kl:', round(val_kl_loss, 4), '- e:',  self.early_stopping_count, '\n')
        ...
        np.savez(self.model_save_path + 'logs_results.npz', log_train_loss=log_train_loss, log_val_loss=log_val_loss)
    #enddef
    ...
    def test(self):
        ...
        print('loss:', round(test_loss, 4), '- rl:', round(test_recon_loss, 4), '- kl:', round(test_kl_loss, 4),'\n')
