...
class GraphBuilderTestCase(unittest.TestCase):
    ...
    def test_rag_builder_with_pipeline_runner(self):
        ...
        config_fname = os.path.join(
            self.current_path, "config", "graph_builder", "rag_graph_builder.yml"
        )
        ...
        self.assertEqual(graph.ndata["feat"].shape[2], 1280)
        # check assigned centroid
        self.assertEqual(graph.ndata["centroid"].shape[1], 2)
    ...
    def test_rag_builder_augmentation_with_pipeline_runner(self):
        ...
        config_fname = os.path.join(
            self.current_path, "config", "graph_builder", "rag_graph_builder_aug.yml"
        )
        with open(config_fname, "r") as file:
            config = yaml.safe_load(file)
        ...
        # self.assertEqual(graph.number_of_edges(), 112)  # check number of edges
        # check assigned features
        self.assertEqual(graph.ndata["feat"].shape[1], 4)
        self.assertEqual(graph.ndata["feat"].shape[2], 1282)
        # check assigned centroid
        self.assertEqual(graph.ndata["centroid"].shape[1], 2)
    ...
    def test_rag_builder(self):
        ...
        slic_extractor = ColorMergedSuperpixelExtractor(
            downsampling_factor=8, superpixel_size=100
        )
        superpixels, _ = slic_extractor.process(image)

        # 3. extract deep features
        feature_extractor = DeepFeatureExtractor(
            architecture="mobilenet_v2", patch_size=224, downsample_factor=2
        )
        features = feature_extractor.process(
            input_image=image, instance_map=superpixels
        )
    ...
    def test_knn_builder_with_pipeline_runner(self):
        ...
        config_fname = os.path.join(
            self.current_path, "config", "graph_builder", "knn_graph_builder.yml"
        )
        with open(config_fname, "r") as file:
            config = yaml.safe_load(file)
    ...
    def test_knn_builder(self):
        ...
        instance_map, instance_centroids = h5_loader._process(
            path=os.path.join(self.nuclei_map_path, self.nuclei_map_name)
        )

        # 3. extract deep features
        feature_extractor = AugmentedDeepFeatureExtractor(
            patch_size=224, architecture="mobilenet_v2", downsample_factor=1
        )
