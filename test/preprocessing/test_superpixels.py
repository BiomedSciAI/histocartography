...
class SuperpixelTestCase(unittest.TestCase):
    ...
    def test_slic_superpixel_extractor_with_pipeline_runner(self):
        ...
        config_fname = os.path.join(
            self.current_path, "config", "superpixels", "slic_extractor.yml"
        )
        ...
        output = pipeline.run(
            output_name=None, image_path=os.path.join(self.image_path, self.image_name)
        )
    ...
    def test_color_merged_superpixel_extractor_with_pipeline_runner(self):
        ...
        config_fname = os.path.join(
            self.current_path, "config", "superpixels", "color_merged_extractor.yml"
        )
        with open(config_fname, "r") as file:
            config = yaml.safe_load(file)
