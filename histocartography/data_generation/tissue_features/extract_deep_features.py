...
class Extract_Deep_Features:
    ...
    def extract_features(self):
        ...
        for i in range(len(img_filepaths)):
            ...
            sp_unmerged_map_path = self.sp_unmerged_detected_path + 'instance_map/' + tumor_type + '/' + basename + '.h5'
            sp_unmerged_centroid_path = self.sp_unmerged_detected_path + 'centroids/' + tumor_type + '/' + basename + '.h5'
            sp_merged_map_path = self.sp_merged_detected_path + 'instance_map/' + tumor_type + '/' + basename + '.h5'

            if os.path.isfile(self.sp_merged_features_path + tumor_type + '/' + basename + '.h5'):
                continue

            if os.path.isfile(sp_unmerged_map_path) and os.path.isfile(sp_merged_map_path):
                ## ----------------------------------------------------------------- Read image information
                ## Image
                img, h, w = self.read_image(img_filepaths[i])
                img_pad = np.pad(img, ((self.patch_size, self.patch_size), (self.patch_size, self.patch_size), (0, 0)), mode='constant', constant_values=-1)

                ## Unmerged SP location
                unmerged_centroids = self.read_centroids(sp_unmerged_centroid_path)

                ## SP instance map
                unmerged_map = self.read_instance_map(sp_unmerged_map_path)
                merged_map = self.read_instance_map(sp_merged_map_path)
                unmerged_map_pad = np.pad(unmerged_map, ((self.patch_size, self.patch_size), (self.patch_size, self.patch_size)), mode='constant', constant_values=-1)

                ## ----------------------------------------------------------------- Extract unmerged features
                unmerged_features = self.extract_unmerged_features(img_pad, unmerged_map_pad, unmerged_centroids)
                h5_fout = h5py.File(self.sp_unmerged_features_path + tumor_type + '/' + basename + '.h5', 'w')
                h5_fout.create_dataset('embeddings', data=unmerged_features, dtype='float32')
                h5_fout.close()

                ## ----------------------------------------------------------------- Extract merged features
                merged_features = self.extract_merged_features(unmerged_map, merged_map, unmerged_features)
                h5_fout = h5py.File(self.sp_merged_features_path + tumor_type + '/' + basename + '.h5', 'w')
                h5_fout.create_dataset('embeddings', data=merged_features, dtype='float32')
                h5_fout.close()
            #endif
        #endfor
    ...
    def extract_merged_features(self, unmerged_map, merged_map, unmerged_features):
