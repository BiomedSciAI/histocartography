...
class Extract_SP:
    ...
    def load_sp_classifier(self):
        self.svm = pickle.load(open(self.sp_classifier_path + 'svm_model.pkl', 'rb'))
    ...
    def classification_merge(self, features, sp_map):
        ...
        sp_map_merged = merge(tissue_id=1, map=sp_map_merged)   # epithelium merging
        sp_map_merged = merge(tissue_id=2, map=sp_map_merged)   # (stroma + necrosis) merging
        sp_map_merged = merge(tissue_id=0, map=sp_map_merged)   # background merging
    ...
    def process_sp(self, img_path, tumor_type):
        ...
        sp_map_unmerged = slic(img, sigma=1, n_segments=n_segments, max_iter=10, compactness=20)
        sp_map_unmerged += 1  # So that no labeled region is 0 and ignored by regionprops, mark_boundaries

        # ----------------------------------------------------- COLOR MERGING
        rag = self.rag_mean_std_color(img, sp_map_unmerged)
        sp_map_merged = graph.merge_hierarchical(sp_map_unmerged, rag, thresh=self.threshold, rag_copy=False,
                                                 in_place_merge=True,
        ...
        sp_map_unmerged = cv2.resize(sp_map_unmerged, (W, H), interpolation=cv2.INTER_NEAREST)
        sp_map_merged = cv2.resize(sp_map_merged, (W, H), interpolation=cv2.INTER_NEAREST)

        # ----------------------------------------------------- CLASSIFIER MERGING
        sp_features_merged = self.extract_sp_classification_features(img_rgb=img_rgb, sp_map=sp_map_merged)
        sp_map_merged = self.classification_merge(features=sp_features_merged, sp_map=sp_map_merged)
        ...
        h5_fout = h5py.File(self.sp_unmerged_detected_path + 'instance_map/' + basename + '.h5', 'w')
        h5_fout.create_dataset('detected_instance_map', data=sp_map_unmerged, dtype='float32')
        h5_fout.close()

        #h5_fout = h5py.File(self.sp_merged_detected_path + 'instance_map/' + tumor_type + '/' + basename + '.h5', 'w')
        h5_fout = h5py.File(self.sp_merged_detected_path + 'instance_map/' + basename + '.h5', 'w')
        h5_fout.create_dataset('detected_instance_map', data=sp_map_merged, dtype='float32')
        h5_fout.close()

        # ----------------------------------------------------- SAVE INSTANCE INFORMATION
        #h5_fout = h5py.File(self.sp_unmerged_detected_path + 'centroids/' + tumor_type + '/' + basename + '.h5', 'w')
        h5_fout = h5py.File(self.sp_unmerged_detected_path + 'centroids/' + basename + '.h5', 'w')
        h5_fout.create_dataset('instance_centroid_location', data=sp_centroid_unmerged, dtype='float32')
        h5_fout.create_dataset('image_dimension', data=img_rgb.shape, dtype='int32')
        h5_fout.close()

        #h5_fout = h5py.File(self.sp_merged_detected_path + 'centroids/' + tumor_type + '/' + basename + '.h5', 'w')
        h5_fout = h5py.File(self.sp_merged_detected_path + 'centroids/' + basename + '.h5', 'w')
        h5_fout.create_dataset('instance_centroid_location', data=sp_centroid_merged, dtype='float32')
        h5_fout.create_dataset('image_dimension', data=img_rgb.shape, dtype='int32')
        h5_fout.close()
