...
class Analyze_Features:
    ...
    def get_filenames(self):
        ...
        for tumor_type in self.tumor_types:
            read_txt_file(self.data_split_dir + 'train_list_' + tumor_type + '.txt', self.train_filenames)
            read_txt_file(self.data_split_dir + 'val_list_' + tumor_type + '.txt', self.val_filenames)
            read_txt_file(self.data_split_dir + 'test_list_' + tumor_type + '.txt', self.test_filenames)
        #endfor
        print('Train:', len(self.train_filenames), ' Val:', len(self.val_filenames), ' Test:', len(self.test_filenames))
    #enddef
    ...
    def prepare_data(self):
        def read_h5_file(filelist):
            ...
            for i in range(len(filelist)):
                ...
                path = self.nuclei_features_path + tumor_type + '/' + filelist[i] + '.h5'
    ...
    def feature_analysis(self):
        ...
        def create_clusters(embeddings):
            if self.clustering_algorithm == 'gmm':
                model = GaussianMixture(n_components=self.nClusters, covariance_type='diag')
                model.fit(embeddings)

            elif self.clustering_algorithm == 'kmeans':
                model = KMeans(n_clusters=self.nClusters, random_state=0).fit(embeddings)
        ...
        sh, ch, db = compute_metrics(self.train_embeddings, train_cluster_labels)
