...
def get_annotation_mask(annotation_file, image_shape, scale_factor=1, all_labels = default_labels):
    ...
    mask_annotated_visualize = np.zeros((image_shape[0], image_shape[1]), np.uint8)
    ...
    if(annotation_file.endswith('.xml')):
        ...
        for j in range(len(Annotation)):
            ...
            if (label not in all_labels): # say if label is empty, then leave it or 3+2 kind
            ...
            mask_annotated = cv2.drawContours(mask_annotated, [ann_coordinates], 0, int(labels[np.where(label == all_labels)]), -1)  # filled with pixel corresponding to roi region
            mask_annotated_visualize = cv2.drawContours(mask_annotated_visualize, [ann_coordinates], 0, int(labels_visualize[np.where(label == all_labels)]), -1)
        ...
        with open(annotation_file, 'r') as f:
            ...
            for row in reader:
                row = np.asarray(row)
                label = str(row[0])
                if (label not in all_labels):  # say if label is empty, the leave it
                    log.debug('{} was continued'.format(label))
                    continue
                log.debug('label : {}'.format(label))
                ...
                mask_annotated = cv2.drawContours(mask_annotated, [ann_coordinates], 0, int(labels[np.where(label == all_labels)]), -1) # filled with pixel corresponding to roi region
                mask_annotated_visualize = cv2.drawContours(mask_annotated_visualize, [ann_coordinates], 0, int(labels_visualize[np.where(label == all_labels)]), -1) # filled with pixel corresponding to roi region
