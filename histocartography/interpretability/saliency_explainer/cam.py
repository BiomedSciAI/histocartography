...
__all__ = ["_CAM", "CAM", "ScoreCAM", "SSCAM"]
EPS = 10e-7
...
class ScoreCAM(_LegacyCAM):
    ...
    def __init__(self, model, conv_layer, input_layer, batch_size=32):
        ...
        # Input hook
        self.hook_handles.append(
            self.model._modules.get(input_layer).register_forward_pre_hook(
                self._store_input
            )
        )
        self.bs = batch_size
    ...
    def _get_weights(self, class_idx, scores=None):
        ...
        # 1 * O * M * N
        upsampled_a = F.interpolate(
            upsampled_a, self._input.shape[-2:], mode="bilinear", align_corners=False
        )
        ...
        # Initialize weights
        weights = torch.zeros(masked_input.shape[0], dtype=masked_input.dtype).to(
        ...
        for idx in range(math.ceil(weights.shape[0] / self.bs)):
            ...
            selection_slice = slice(
                idx * self.bs, min((idx + 1) * self.bs, weights.shape[0])
            )
            with torch.no_grad():
...
class SSCAM(ScoreCAM):
    ...
    def __init__(
        self, model, conv_layer, input_layer, batch_size=32, num_samples=35, std=2.0
    ):
    ...
    def _get_weights(self, class_idx, scores=None):
        ...
        # Initialize weights
        weights = torch.zeros(upsampled_a.shape[0], dtype=upsampled_a.dtype).to(
