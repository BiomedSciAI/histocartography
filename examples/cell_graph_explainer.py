...
def explain_cell_graphs(cell_graph_path, image_path):
    ...
    # 2. create model 
    config_fname = os.path.join(os.path.dirname(__file__), 'config', 'cg_bracs_cggnn_3_classes_gin.yml')
    ...
    for cg_path in tqdm(cg_fnames):
        ...
        image_path = [x for x in image_fnames if graph_name in x.replace('.png', '.bin')][0]
