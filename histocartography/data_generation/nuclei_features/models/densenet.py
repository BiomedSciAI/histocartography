...
class DenseNet(nn.Module):
    ...
    def get_layers(self):
        if type(self._layers) is not list:
            if self._layers == -1:
                assert (self._depth - 4) % 3 == 0, 'Depth must be 3 N + 4 if layers == -1'
    ...
    def _build_init(self):
        block = nn.Sequential(
            nn.Conv2d(3, self._initial_filters, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(self._initial_filters),
            nn.ReLU())
    ...
    def _build_center(self):
        ...
        for idx, num_layers in enumerate(self._layers):
            center.append(DenseBlock(growth_rate=self._growth_rate, num_layers=num_layers, num_input_features=num_features))
            num_features = num_features + num_layers * self._growth_rate

            if idx != len(self._layers) - 1:
                center.append(self._transition_layer(num_input_features=num_features, num_output_features=num_features // 2))
...
class DenseBlock(nn.Module):
    ...
    def _DenseLayer(self, num_input_features):
        block = nn.Sequential(
            nn.BatchNorm2d(num_input_features),
            nn.ReLU(),
            nn.Conv2d(num_input_features, self._growth_rate * 4, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(self._growth_rate * 4),
            nn.ReLU(),
            nn.Conv2d(self._growth_rate * 4, self._growth_rate, kernel_size=3, stride=1, padding=1, bias=False))
        return block
    #enddef
#end
