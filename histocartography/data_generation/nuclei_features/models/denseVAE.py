...
class DenseVAE(nn.Module):
    ...
    def __init__(self, embedding_dim, encoder_layers_per_block, patch_size, device):
        super(DenseVAE, self).__init__()

        self._embedding_dim = embedding_dim
        self._encoder_layers_per_block = encoder_layers_per_block
        self._encoder_nb_blocks = 5
        self._encoder_image_size = int(patch_size/2**(self._encoder_nb_blocks -1))
        self._device = device
    ...
    def _build_layers(self):
        ...
        self._upsample_filters = [ele for ele in reversed(self._upsample_filters)]

        self._encoder_output_dim = self._upsample_filters[0] * self._encoder_image_size * self._encoder_image_size
        ...
        self._decoder_fc = nn.Linear(self._embedding_dim, self._encoder_output_dim)
...
def dense_vae(embedding_dim, encoder_layers_per_block=1, patch_size=72, device=None):
    model = DenseVAE(embedding_dim=embedding_dim, encoder_layers_per_block=encoder_layers_per_block, patch_size=patch_size, device=device)
    return model
#ennddef
...
if __name__ == '__main__':
    ...
    model = dense_vae(embedding_dim=16, encoder_layers_per_block=1, patch_size=72)
