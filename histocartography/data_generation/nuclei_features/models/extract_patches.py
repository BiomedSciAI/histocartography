...
class Extract_Patches:
    def __init__(self, config):
        ...
        self.test_exclude_list = ['283_benign_19_explanation', '1231_pathologicalbenign_16_explanation']
    ...
    def get_filenames(self, path):
        ...
        nuclei_detected_path = self.nuclei_detected_path + tumor_type + '/_h5/' + filename + '.h5'
    ...
    def extract_patches(self, mode):
        ...
        if mode != 'test':
            explanation_paths = glob.glob(self.explanation_path + mode + '/fold_4/*.json')
        else:
            explanation_paths = glob.glob(self.explanation_path + mode + '/fold_4/*explanation.json')
        ...
        for i in range(len(explanation_paths)):
            basename, filename, tumor_type, tumor_index, img_path, nuclei_info_path = self.get_filenames(explanation_paths[i])
            ...
            if os.path.isfile(img_path) and os.path.isfile(nuclei_info_path):
                ...
                idx = self.get_centroid_index(mode, tumor_index, len(centroids), seed=i)
                ...
                for j in range(len(idx)):
                    ...
                    if (y - self.patch_size_2 > 0) and (y + self.patch_size_2 < h) and (x - self.patch_size_2 > 0) and (x + self.patch_size_2 < w):
                        nuclei_count += 1
                        patch = img[y - self.patch_size_2: y + self.patch_size_2, x - self.patch_size_2: x + self.patch_size_2, :]
                        Image.fromarray(patch).save(self.vae_patches_path + mode + '/' + basename + '_'  + str(j) + '.png')

                        mask_ = nuclei_instance_map[y - self.patch_size_2: y + self.patch_size_2, x - self.patch_size_2: x + self.patch_size_2]
                        mask = np.zeros_like(mask_)
                        mask[mask_ == idx[j] + 1] = 255
                        mask[mask_ != idx[j] + 1] = 0
                        mask = mask.astype(np.uint8)
                        mask = cv2.dilate(mask, self.kernel, iterations=1)
                        Image.fromarray(mask).save(self.vae_masks_path + mode + '/' + basename + '_' + str(j) + '.png')
                    else:
                ...
                print(i, 'File name=', filename, ' #nuclei=', nuclei_count, ' #boundary_nuclei=', boundary_nuclei_count)
            #endif
        #endfor

        print('\nTUMOR-WISE COUNT:\n')
        for i in range(len(self.tumor_types)):
            print(self.tumor_types[i], ': #troi=', tumor_count[i], ' #nuclei', tumor_nuclei_count[i])
    #enddef
#end
