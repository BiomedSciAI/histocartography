...
def get_patches(image_id, image=None, patch_size=None, visualize=1):
    ...
    cv2.imwrite(os.path.join(str(image_id) + '_downsample_image.png'), downsample_image)
    ...
    while ((x_0 + patch_size) <= image.shape[0]):
        while ((y_0 + patch_size) <= image.shape[1]):
            ...
            patch_mask = binary_tissue_mask[x_0: x_0 + patch_size, y_0: y_0 + patch_size]

            tissue_percent_in_patch = (np.count_nonzero(patch_mask == labels[1]) * 100) /(patch_size ** 2)
    ...
    log.debug('Total number of patches: {}'.format(len(patch_info_coordinates)))
    ...
    if(visualize==1):
        ...
        Image_ = downsample_image # Stack.read_region((0, 0), 3, (size_3[0], size_3[1])) # 5x
        Image_ = Image.fromarray(Image_)
        draw0 = ImageDraw.Draw(Image_, 'RGBA')

        for k, val in enumerate(patch_info_coordinates_visualize):
            draw0.rectangle(val[1:], outline=box_color)  # , fill=box_color)#(255, 0, 0, 256))

        Image_ = Image_.convert('RGB')
        Image_ = np.asarray(Image_)
        cv2.imwrite(os.path.join(str(image_id) + '_patches_outlines.png'), Image_)
