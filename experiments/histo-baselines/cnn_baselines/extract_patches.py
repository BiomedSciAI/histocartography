...
class Extract_Patches:
    def __init__(self, config, is_balanced):
        ...
        if self.is_balanced:
            ...
            self.stride = [self.patch_size_10x for i in range(
                len(self.tumor_types))]
        ...
        for t in range(len(self.tumor_types)):
            ...
            for i in range(len(img_paths)):
                ...
                if h < self.patch_size_10x:
                    img = np.pad(img, ((0, self.patch_size_10x), (0, 0),
                                       (0, 0)), mode='constant', constant_values=255)

                if w < self.patch_size_10x:
                    img = np.pad(img, ((0, 0), (0, self.patch_size_10x),
                                       (0, 0)), mode='constant', constant_values=255)

                counter = self.select_patches(
                    imgname, img, self.tumor_types[t])
                if counter == 0:
        ...
        while (x + self.patch_size_10x) <= w:
            ...
            while (y + self.patch_size_10x) <= h:
                ...
                img_40x = img[y +
                              self.offset_40x: y +
                              self.offset_40x +
                              self.patch_size_40x, x +
                              self.offset_40x: x +
                              self.offset_40x +
                              self.patch_size_40x, :]

                Image.fromarray(img_10x).save(self.base_patches_path +
                                              tumor_type +
                                              '/10x/' +
                                              imgname +
                                              '_' +
                                              ...
                                              '_' +
                                              str(x +
                                                  self.offset_centroid) +
                                              '_' +
                                              str(y +
                                                  self.offset_centroid) +
                                              '.png')

                Image.fromarray(img_20x).save(self.base_patches_path +
                                              tumor_type +
                                              '/20x/' +
                                              imgname +
                                              '_' +
                ...
                Image.fromarray(img_40x).save(self.base_patches_path +
                                              tumor_type +
                                              '/40x/' +
                                              imgname +
                                              '_' +
