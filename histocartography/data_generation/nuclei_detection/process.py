...
class Process:
    ...
    def overlaid_output(self, img, pred_inst, pred_centroid, basename):
        ...
        for item_t in pred_centroid:
            j += 1
            cv2.drawMarker(overlaid_output,
                           (int(item_t[0]),
                            int(item_t[1])),
                           (0,
                               255,
            ...
            cv2.putText(
                overlaid_output, str(j), (int(
                    item_t[0]), int(
                    item_t[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255))  # , 2, cv2.LINE_AA)
        # endfor
        cv2.imwrite(
            '%s/%s.png' %
            (self.proc_overlaid_dir,
             basename),
            overlaid_output)
    # enddef
    ...
    def run(self):
        ...
        for i in range(len(file_list)):
            ...
            if os.path.isfile(
                    self.centroid_output_dir +
                    basename +
                    '.h5') and os.path.isfile(
                    self.proc_h5_dir +
                    basename +
                    '.h5'):
                print('#', i, basename)
            ...
            if hasattr(
                    self,
                    'type_classification') and self.type_classification:
            ...
            if len(pred_centroid) == 0:
                ...
                for i, region in enumerate(regions):
                    # (row, column) = (y, x)
                    pred_centroid[i, :] = region['centroid']
            ...
            if self.type_classification:
                pred_id_list = list(np.unique(pred_inst))[
                    1:]  # exclude background ID
                pred_inst_type = np.full(len(pred_id_list), 0, dtype=np.int32)
                for idx, inst_id in enumerate(pred_id_list):
                    inst_type = pred_type[pred_inst == inst_id]
                    type_list, type_pixels = np.unique(
                        inst_type, return_counts=True)
                    type_list = list(zip(type_list, type_pixels))
                    type_list = sorted(
                        type_list, key=lambda x: x[1], reverse=True)
                ...
                save_centroid_h5(
                    ...
                    pred_centroid,
                    img.shape)
                save_instance_map_h5(
                    self.proc_h5_dir + basename + '.h5', pred_inst)
                save_class_type_h5(self.proc_class_dir +
                                   basename +
                                   '.h5', pred_type, pred_inst_type[:, None])
